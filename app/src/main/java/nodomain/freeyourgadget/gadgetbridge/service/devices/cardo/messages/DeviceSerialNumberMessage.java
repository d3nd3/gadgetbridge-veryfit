...
public class DeviceSerialNumberMessage extends AbstractStringMessage {
    public DeviceSerialNumberMessage(byte[] payload) {
        super(payload);
    }

    public CardoMap<ByteUtils.CardoField, Object> getDeviceStatus() {
        final CardoMap<ByteUtils.CardoField, Object> temp = new CardoMap<>();
        temp.put(new ByteUtils.CardoField("serial", alias.length()), alias);
        return temp;
    }
}
